...
void CategoryAnalyzer::run()
{
	...
	ci.classPointerAddress = ci.address + 0x8;
        ci.classPointer = arp(m_file->readLong(ci.classPointerAddress));

        ci.methodListAddress = arp(m_file->readLong(ci.address + 0x10));
	...
	if (ci.classMethodListAddress)
	    ci.classMethodList = analyzeMethodList(ci.classMethodListAddress);
